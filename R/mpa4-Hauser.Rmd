...
gtdb_table <- read_tsv(file.path(file_path, "gtdb_merged_table.txt"), comment = "#")
...
load_mpa <- function(gtdb_table, level) {
  ...
  if (level == "phylum") {
    ...
    colnames(gtdb_table) <- gsub("-report-Family\\.breport$", "", colnames(gtdb_table))
  } 
  else if (level == "genera") {
    gtdb_table <- gtdb_table %>%
      filter(str_detect(clade_name, "g__") & !str_detect(clade_name, "s__")) %>%
      mutate(clade_name = sub(".*g__", "", clade_name))
    colnames(gtdb_table) <- gsub("-report-Genera\\.breport$", "", colnames(gtdb_table))
  } 
  else if (level == "species") {
    gtdb_table <- gtdb_table %>%
      filter(str_detect(clade_name, "s__")) %>%
      mutate(clade_name = sub(".*s__", "", clade_name))
    colnames(gtdb_table) <- gsub("-report-Species\\.breport$", "", colnames(gtdb_table))
  } 
  else {
