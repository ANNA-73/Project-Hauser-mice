...
dist_long <- dist_matrix %>%
  ...
  filter(as.numeric(Mouse1) > as.numeric(Mouse2))
...
p_top10_post_overall <- ggplot(top10_post_overall,
             aes(x = reorder(clade_name, -mean_abundance),
                 y = mean_abundance * 100)) +
  geom_bar(stat = "identity", fill = "#228B22", width = 0.7) +
  labs(
...
# ============================================================================
# 2. TOP 10 MOST ABUNDANT GENERA - BY TREATMENT GROUP
# ============================================================================
...
# ============================================================================
# 3. TEMPORAL DYNAMICS - BOXPLOTS
# ============================================================================

# Filter for top 10 overall genera
post_top10_data <- post_treatment_data %>%
  filter(clade_name %in% top10_post_overall$clade_name)

p_temporal_boxplot <- ggplot(post_top10_data, 
             aes(x = as.factor(Day), y = Relative_Abundance * 100, fill = Treatment)) +
  geom_boxplot(outlier.size = 1) +
  facet_wrap(~clade_name, scales = "free_y", ncol = 5) +
  scale_fill_brewer(palette = "Set1") +
  labs(
    title = "Temporal Dynamics of Top 10 Genera",
    subtitle = "Sample-level variation shown, grouped by treatment",
    x = "Day",
    y = "Relative Abundance (%)",
    fill = "Treatment"
  ) +
...
print(p_temporal_boxplot)

# ============================================================================
# 4. TEMPORAL DYNAMICS - LINE PLOTS WITH MEAN ± SE
# ============================================================================

# Calculate mean and SE for each genus, treatment, day
temporal_summary <- post_top10_data %>%
  group_by(clade_name, Treatment, Day) %>%
  summarize(
...
p_temporal_lines <- ggplot(temporal_summary, 
             aes(x = Day, y = mean_abundance * 100, color = Treatment, group = Treatment)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = (mean_abundance - se) * 100, 
                    ymax = (mean_abundance + se) * 100), 
                width = 0.5, alpha = 0.7) +
  facet_wrap(~clade_name, scales = "free_y", ncol = 5) +
  scale_color_brewer(palette = "Set1") +
  labs(
    title = "Temporal Dynamics of Top 10 Genera (Mean ± SE)",
    subtitle = "Trajectories over time by treatment group",
    x = "Day",
    y = "Mean Relative Abundance (%)",
    color = "Treatment"
  ) +
...
print(p_temporal_lines)

# ============================================================================
# 5. INDIVIDUAL MOUSE TRAJECTORIES (for key genera)
# ============================================================================

top4_genera <- top10_post_overall$clade_name[1:4]

# Calculate mean and SD for each genus, treatment, day
temporal_summary_top4 <- post_treatment_data %>%
  filter(clade_name %in% top4_genera) %>%
  group_by(clade_name, Treatment, Day) %>%
  summarize(
    mean_abundance = mean(Relative_Abundance),
...
# Plot with lines for treatments
p_temporal_top4 <- ggplot(temporal_summary_top4, 
             aes(x = Day, y = mean_abundance * 100, 
                 color = Treatment, group = Treatment)) +
  geom_line(linewidth = 1.2) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = (mean_abundance - sd_abundance) * 100, 
                    ymax = (mean_abundance + sd_abundance) * 100), 
                width = 0.8, linewidth = 0.8, alpha = 0.7) +
  facet_wrap(~clade_name, scales = "free_y", ncol = 2) +
  scale_color_brewer(palette = "Set1") +
  scale_x_continuous(breaks = c(0, 3, 10, 14)) +
  labs(
    title = "Temporal Dynamics of Top 4 Most Abundant Genera",
    subtitle = "Mean ± SD by treatment group",
    x = "Day",
    y = "Mean Relative Abundance (%)",
    color = "Treatment"
  ) +
...
annotation_colors <- list(
  Treatment = c(CK = "#E41A1C", CZ = "#377EB8", V = "#4DAF4A", ZK = "#984EA3"),
