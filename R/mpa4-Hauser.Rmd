...
custom_colors <- c("#26456E", "#1c54a8", "#FFE04F", "#EC6E1C", "#9F3632")
...
print(p_distance_heatmap)
 ...
 ggsave("baseline_distance_heatmap.png", p_distance_heatmap,
        path = plots_path, width = 14, height = 12, dpi = 300)
...
p_distance_heatmap <- ggplot(dist_long, aes(x = Mouse2, y = Mouse1, fill = Distance)) +
  ...
  scale_fill_gradientn(colors = custom_colors, limits = c(0,1) ) +
  labs(
    title = "Pairwise Bray-Curtis Distances Between Mice at Day14",
    subtitle = paste0("Mean Distance = ", mean_dist),  # DYNAMIC
    x = "Mouse ID",
    y = "Mouse ID",
    fill = "Bray-Curtis\nDissimilarity"
...
ggsave(
  filename = file.path(plots_path, "day14_distance_heatmap.png"),
...
core_genera_filtered <- core_genera_filtered %>%
  mutate(Day = factor(Day, levels = c(-5, 0, 3, 10, 14)))

core_species_filtered <- core_species_filtered %>%
  mutate(Day = factor(Day, levels = c(-5, 0, 3, 10, 14)))
...
p_baseline_mouse <- pheatmap(
  ...
  clustering_method = "complete",
  cluster_cols = TRUE,
  annotation_col = mouse_treatment,  
  annotation_colors = list(
    Treatment = c("CK" = "#3770AC", "CZ" = "#9E421E", "V" = "#8C8C8C", "ZK" = "#327341")
  ),
  color = viridis(100),
...
col_fun <- colorRamp2(
  breaks = seq(-2, 1.73, length.out = 100),
  colors = viridis(100)
)

# Column annotation with THICK bars and LARGE fonts
ha_col <- HeatmapAnnotation(
  Treatment = col_annotation$Treatment,
  Day = as.character(col_annotation$Day),
  col = list(
    Treatment = c("CK" = "#3770AC", "CZ" = "#9E421E", "V" = "#8C8C8C", "ZK" = "#327341"),
    Day = c("-5" = "#feedde", "0" = "#fdbe85", "3" = "#fd8d3c", "10" = "#e6550d", "14" = "#a63603")
  ),
  annotation_height = unit(c(10, 10), "mm"),           # THICK annotation bars
  annotation_name_gp = gpar(fontsize = 16, fontface = "bold"),  # Annotation label font
...
# Option 1: Move row names to the left side
ht <- Heatmap(
  ...
  column_split = col_annotation$Day,
  column_gap = unit(4, "mm"),
  
  # MOVE ROW NAMES TO LEFT
  row_names_side = "left",
  ...
  column_title = "Genus Relative Abundance Across All Days - Log10(%)",
...
# Draw with more padding
png(file.path(plots_path, "all_days_heatmap.png"), 
    width = 22, height = 16, units = "in", res = 300)

draw(ht, 
     heatmap_legend_side = "right",
     annotation_legend_side = "right",
     padding = unit(c(20, 50, 20, 20), "mm"),
     merge_legend = TRUE,
     legend_gap = unit(20, "mm")
)
